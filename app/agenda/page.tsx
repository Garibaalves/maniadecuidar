...
export default function AgendaPage() {
  ...
  const valorTotal = useMemo(() => {
    return (form.servicos ?? []).reduce((acc, item) => {
      ...
      const valor =
        item.valor != null && !Number.isNaN(item.valor)
          ? item.valor
          : base?.valor_padrao ?? 0;
      return acc + valor;
    }, 0);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.cliente_id || !form.animal_id || !form.hora) {
      setError("Preencha cliente, animal e horário");
      return;
    }
    setSaving(true);
    setError(null);
  ...
  return (
    <AppShell>
      <PageHeader
        title="Agenda"
        description="Visualização diária/semanal, filtros e ações rápidas de atendimento."
        actions={
          <>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-brand-primary/20 md:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between gap-3">
            <div>
              <CardTitle>Dia {hojeFormatado}</CardTitle>
              <p className="text-sm text-foreground/70">
                Horários reais da configuração e agendamentos existentes.
              </p>
            </div>
